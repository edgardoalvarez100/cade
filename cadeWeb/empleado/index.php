        ...
        <div class="contentarea" id="contentarea">
            <div style="float:left;width:100%;">
                ...
                <div style="margin-bottom:15px;">
                    <div id="sysoverviewbanner">
                        <div style="margin:0 0 -5px 0;padding: 10px;background-color: #FBEEEB;border: 1px dashed #cc0000;font-weight: bold;color: #cc0000;font-size:14px;text-align: center;-moz-border-radius: 10px;-webkit-border-radius: 10px;-o-border-radius: 10px;border-radius: 10px;">Elementos de atención: &nbsp;
                            <?php
                            $sql = "select count(*) from tickets t inner join estadotickets e ON t.idestado=e.idestado 
INNER JOIN categorias c ON c.idcategoria=t.idcategoria 
INNER JOIN  usuarios u ON t.idusuario=u.idusuario 
WHERE e.estado='SIN RESPONDER' and c.idcategoria IN (select c.idcategoria from usuarios u inner JOIN categoriasempleados ce ON u.idusuario=ce.idusuario 
INNER JOIN categorias c ON c.idcategoria=ce.idcategoria where u.idusuario=".$_SESSION["IdUsuario"].") ";
                            ...
                            ?>
                            Ticket(s) esperando respuesta &nbsp;</div>
                    </div>
                </div>
                
                    <h1>Tickets Esperando Respuesta</h1>
                    <div class="tablebg">
                        <table id="sortabletbl2" class="datatable" width="100%" border="0" cellspacing="1" cellpadding="3">
                            ...
                            <?php
                            $sql = "select * from tickets t inner join estadotickets e ON t.idestado=e.idestado 
INNER JOIN categorias c ON c.idcategoria=t.idcategoria 
INNER JOIN  usuarios u ON t.idusuario=u.idusuario 
WHERE e.estado='SIN RESPONDER' and c.idcategoria IN (select c.idcategoria from usuarios u inner JOIN categoriasempleados ce ON u.idusuario=ce.idusuario 
INNER JOIN categorias c ON c.idcategoria=ce.idcategoria where u.idusuario=".$_SESSION["IdUsuario"].")";
                            ...
                            if (mysql_num_rows($reporte) > 0) {
                                while ($rep = mysql_fetch_array($reporte)) {
                                    ...
                                    echo "<tr>
                       <td>$i</td>
						<td><a href='responder.php?id=" . $rep['idticket'] . "'>#" . $rep['idticket'] . "</a></td>
						<td>" . $rep['nombre'] . " " . $rep['apellidos'] . "</td>
						<td><div style='text-align:left;'><a href='responder.php?id=" . $rep['idticket'] . "'>#" . $rep['idticket'] . " - " . $rep['asunto'] . "</a></div></td>
                        <td>" . $rep['categoria'] . "</td>
                        <td>" . $rep['fecha'] . "</td>
